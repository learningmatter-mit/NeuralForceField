...
def obfit_rmsd(file1, file2, smarts, path):
    cmd = ["obfit", "'" + str(smarts) + "'", os.path.join(path, file1 + ".xyz"), os.path.join(path, file2 + ".xyz")]
...
class ConformerGenerator(object):
    ...
    def generate(
        self, max_generated_conformers=50, prune_thresh=0.01, maxattempts_per_conformer=5, output=None, threads=1
    ):
        """
    ...
    def cluster(self, rms_tolerance=0.1, max_ranked_conformers=10, energy_window=5, Report_e_tol=10, output=None):
        """
        ...
        for conf_a in clustered_confs:
            ...
            for conf_b in clustered_confs[i + 1:]:
                ...
                if j in self.removed:
                    continue
                try:
                    rms = obfit_rmsd(key + "_Conf_" + str(i + 1), key + "_Conf_" + str(j + 1), str(molecule), path=path)
                ...
                if rms > rms_tolerance:
                    ...
                    filename = os.path.join(path, key + "_Conf_" + str(j + 1) + "_inv.xyz")
                    write_xyz(coords=coords, filename=filename, comment=conf_b[1])
                    try:
...
def minimize(output, molecule, forcefield, nconf_gen, prun_tol, e_window, rms_tol, rep_e_window):
...
def write_clusters(output, idx, conformer, inchikey, path):
...
def run_obabel(inchikey, idx):
    ...
    molecule = subprocess.check_output(
        cmd, stdin=None, stderr=subprocess.STDOUT, shell=False, universal_newlines=False
    ).decode("utf-8")
    molecule = clean(molecule)
    return molecule


def summarize(output, gen_time, start_time, min_time, cluster_time):
...
def update_with_boltz(geom_list, temp):
...
def make_geom_dic(lines, smiles, geom_list, idx):
    ...
    if idx == 0:
        ...
        ref_energy_kcal = geom_list[0]["totalenergy"] * AU_TO_KCAL
        rel_energy = energy_kcal - ref_energy_kcal

    rd_mol = xyz_to_rdmol(nxyz=nxyz, smiles=smiles)
...
def combine_geom_dics(geom_list, temp, other_props, smiles):
    totalconfs = sum([i["degeneracy"] for i in geom_list])
    uniqueconfs = len(geom_list)
    lowestenergy = geom_list[0]["totalenergy"]
    poplowestpct = geom_list[0]["boltzmannweight"] * 100
    charge = get_charge(smiles)

    combination = {
        "totalconfs": totalconfs,
...
def parse_results(job_dir, log_file, inchikey, smiles, max_confs, other_props, temp, clean_up):
    # import pdb
    # pdb.set_trace()
    ...
    if loglines[-1].strip() != "Terminated successfully":
        msg = "'Terminated successfully' not found " "at end of conformer output"
        raise Exception(msg)
    ...
    for i in range(max_confs):
        ...
        geom = make_geom_dic(lines=lines, smiles=smiles, geom_list=geom_list, idx=i)
        geom_list.append(geom)

    geom_list = update_with_boltz(geom_list=geom_list, temp=temp)
    summary_dic = combine_geom_dics(geom_list=geom_list, temp=temp, other_props=other_props, smiles=smiles)
...
def one_species_confs(
    ...
    with open(log, "w") as output:
        ...
        inchikey = inchi.MolToInchiKey(get_mol(molecule), options=INCHI_OPTIONS)

        for i, conformer in enumerate(clustered_confs):
            write_clusters(output=output, idx=i, conformer=conformer, inchikey=inchikey, path=job_dir)
        ...
        summarize(output=output, gen_time=gen_time, start_time=start_time, min_time=min_time, cluster_time=cluster_time)

    conf_dic = parse_results(
        job_dir=job_dir,
        log_file=log_file,
        inchikey=inchikey,
        max_confs=max_confs,
...
def add_to_summary(summary_dic, conf_dic, smiles, save_dir):
    inchikey = inchi.MolToInchiKey(get_mol(smiles), options=INCHI_OPTIONS)
    pickle_path = os.path.join(os.path.abspath(save_dir), f"{inchikey}.pickle")
    summary_dic[smiles] = {key: val for key, val in conf_dic.items() if key != "conformers"}
...
def confs_and_save(config_path):
    ...
    for i, smiles in tqdm_enum(smiles_dic["smiles"]):
        other_props = {key: val[i] for key, val in smiles_dic.items() if key != "smiles"}
        smiles_list = [smiles]
        conf_dic = run_generator(smiles_list=smiles_list, other_props=other_props, **info)[0]
        summary_dic, pickle_path = add_to_summary(
            summary_dic=summary_dic, conf_dic=conf_dic, smiles=smiles, save_dir=save_dir
        )
