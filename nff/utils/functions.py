...
def A_m(x, y, m):
    ...
    binoms = torch.Tensor([sp.binom(m, int(p)) for p in p_vals]).to(device)
...
def pi_l_m(r, z, l, m, c_table):
    device = r.device
    pref = (sp.factorial(l - m) / sp.factorial(l + m)) ** 0.5
    p_vals = torch.arange(0, math.floor((l - m) / 2) + 1, device=device, dtype=torch.float)

    c_coefs = torch.Tensor([c_table[(int(p), l, m)] for p in p_vals]).to(device)
...
def y_lm(r_ij, r, l, m, c_table):
    x = r_ij[:, 0].reshape(-1, 1)
    y = r_ij[:, 1].reshape(-1, 1)
    z = r_ij[:, 2].reshape(-1, 1)

    pi = pi_l_m(r=r, z=z, l=l, m=abs(m), c_table=c_table)
...
def make_y_lm(l_max):
    ...
    def func(r_ij, r, l, m):
        out = y_lm(r_ij=r_ij, r=r, l=l, m=m, c_table=c_table)
...
def spooky_f_cut(r, r_cut):
    ...
    output = torch.where((r < r_cut) * (arg < 20), torch.exp(-arg), torch.Tensor([0]).to(r.device))
...
def b_k(x, bern_k):
    device = x.device
    k_vals = torch.arange(0, bern_k, device=device).to(torch.float)
    binoms = torch.Tensor([sp.binom(bern_k - 1, int(k)) for k in k_vals]).to(device)
    out = binoms * (x**k_vals) * (1 - x) ** (bern_k - 1 - k_vals)
    return out


def rho_k(r, r_cut, bern_k, gamma):
...
def get_g_func(l, r_cut, bern_k, gamma, y_lm_fn):
    def fn(r_ij):
        ...
        y = torch.stack([y_lm_fn(r_ij, r, l, m) for m in m_vals]).transpose(0, 1)
        rho = rho_k(r, r_cut, bern_k, gamma)
        g = torch.ones(n_pairs, bern_k, len(m_vals), device=device)
...
def make_g_funcs(bern_k, gamma, r_cut, l_max=2):
