...
def get_main_parser():
    ...
    cmd_parser.add_argument(
        "--parallel",
        help="Run data-parallel on all available GPUs (specify with environment" " variable CUDA_VISIBLE_DEVICES)",
        action="store_true",
    )
    cmd_parser.add_argument(
        "--batch_size",
...
def add_subparsers(cmd_parser, defaults={}):
    ...
    schnet_parser.add_argument("--n_filters", type=int, help="Size of atom-wise representation", default=25)
    schnet_parser.add_argument(
        "--n_gaussians",
        type=int,
        default=25,
    ...
    schnet_parser.add_argument("--n_convolutions", type=int, help="Number of interaction blocks", default=6)
    schnet_parser.add_argument(
        "--cutoff",
        type=float,
        default=5.0,
    ...
    cmd_subparsers = cmd_parser.add_subparsers(dest="mode", help="Command-specific arguments")
    ...
    subparser_export.add_argument("dest_path", help="Destination path for exported model")
