...
def compute_gap(results, lower_key, upper_key):
    lower_e = results[lower_key]
    upper_e = results[upper_key]
    gap = upper_e - lower_e

    return gap


def compute_sum(results, lower_key, upper_key):
    lower_e = results[lower_key]
    upper_e = results[upper_key]
    sm = upper_e + lower_e
...
def compute_gap_grad(results, lower_key, upper_key):

    out = compute_gap(results=results,
                      lower_key=lower_key + "_grad",
                      upper_key=upper_key + "_grad")
    return out


def compute_sum_grad(results, lower_key, upper_key):

    out = compute_sum(results=results,
                      lower_key=lower_key + "_grad",
                      upper_key=upper_key + "_grad",
                      )
    return out
...
def compute_gap_penalty(results, lower_key, upper_key,
                        ref_energy, penalty):

    gap = compute_gap(results=results, lower_key=lower_key,
                      upper_key=upper_key)

    sm = compute_sum(results=results, lower_key=lower_key,
                     upper_key=upper_key)

    out = gap + penalty * (sm / 2 - ref_energy)

    return out


def compute_gap_penalty_grad(results, lower_key, upper_key, penalty):
    out = compute_gap_penalty(
        results=results,
        lower_key=lower_key + "_grad",
        upper_key=upper_key + "_grad",
        ref_energy=0,
...
def get_process_func(name, **kwargs):
    dic = {"gap": compute_gap,
           "gap_grad": compute_gap_grad,
           "gap_penalty": compute_gap_penalty,
           "gap_penalty_grad": compute_gap_penalty_grad}

    # def func(results): return dic[name](results=results, **kwargs)
    ...
    return kwargs["penalty"], kwargs.get("ref_energy")


def make_pp_funcs(process_list):

    process_funcs = []

    for dic in process_list:
        func_name = dic["func_name"]
        kwargs = dic["params"]

        func = get_process_func(func_name, **kwargs)
