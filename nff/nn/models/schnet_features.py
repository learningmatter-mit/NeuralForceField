...
class AttentionNet(SchNetFeatures):

    def __init__(self, modelparams):

        SchNetFeatures.__init__(self, modelparams)
        self.conf_att = self.make_att_net(modelparams)

    def make_att_net(self, modelparams):
        ...
        net = LinearConfAttention(mol_basis=mol_basis,
    ...
    def get_weights(self, outputs):
        ...
        for i in range(len(conf_fps_by_smiles)):
            ...
            weights = self.conf_att(conf_fps, boltzmann_weights)
    ...
    def forward(self, batch, xyz=None, **kwargs):
        ...
        all_weights = self.get_weights(outputs)
