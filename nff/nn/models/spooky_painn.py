...
from nff.nn.modules.schnet import AttentionPool, MeanPool, MolFpPool, SumPool, get_offsets, get_rij
...
POOL_DIC = {"sum": SumPool, "mean": MeanPool, "attention": AttentionPool, "mol_fp": MolFpPool}
...
class SpookyPainn(Painn):
    def __init__(self, modelparams):
        ...
        Painn.__init__(self, modelparams)
        ...
        self.embed_block = CombinedEmbedding(feat_dim=feat_dim, activation=activation)
        ...
        self.electrostatics = nn.ModuleDict(
            {
                key: PainnElectrostatics(feat_dim=feat_dim, activation=activation, r_cut=cutoff, **elec_terms)
                for key in add_elec_keys
            }
        )

        self.nuc_repulsion = nn.ModuleDict({key: NuclearRepulsion(r_cut=cutoff) for key in add_nuc_keys})

        if add_disp_keys:
            raise NotImplementedError("Dispersion not implemented")
        self.cutoff = cutoff

    def atomwise(self, batch, nbrs, num_atoms, xyz=None):
        ...
        s_i, v_i = self.embed_block(charge=charge, spin=spin, z=z_numbers, num_atoms=num_atoms)
        results = {}

        for i, message_block in enumerate(self.message_blocks):
            update_block = self.update_blocks[i]
            ds_message, dv_message = message_block(s_j=s_i, v_j=v_i, r_ij=r_ij, nbrs=nbrs, num_atoms=num_atoms.tolist())
    ...
    def add_phys(self, results, s_i, v_i, xyz, z, charge, nbrs, num_atoms, offsets, mol_offsets, mol_nbrs):
        ...
        for key in self.output_keys:
            ...
            if key in nuc_repulsion:
                ...
                nuc_e = nuc_module(xyz=xyz, z=z, nbrs=nbrs, num_atoms=num_atoms, offsets=offsets)
    ...
    def pool(self, batch, atomwise_out, xyz, nbrs, num_atoms, z, s_i, v_i):
        ...
        for key, pool_obj in self.pool_dic.items():
            results = pool_obj(batch, xyz, r_ij, nbrs, atomwise_output=atomwise_out, grad_keys=[], out_keys=[key])
    ...
    def run(self, batch, xyz=None, **kwargs):
        nbrs, _ = make_directed(batch["nbr_list"])
        num_atoms = batch["num_atoms"]
        z = batch["nxyz"][:, 0].long()

        atomwise_out, xyz, s_i, v_i = self.atomwise(batch=batch, xyz=xyz, nbrs=nbrs, num_atoms=num_atoms)
        all_results, xyz = self.pool(
            batch=batch, atomwise_out=atomwise_out, xyz=xyz, nbrs=nbrs, num_atoms=num_atoms, z=z, s_i=s_i, v_i=v_i
        )
...
class SpookyPainnDiabat(SpookyPainn):
    def __init__(self, modelparams):
        ...
        others_to_eig = [get_others_to_eig(diabat_keys)] if self.electrostatics else None
    ...
    def forward(self, batch, xyz=None, add_nacv=False, add_grad=True, add_gap=True):
        ...
        results = self.diabatic_readout(
            batch=batch, xyz=xyz, results=diabat_results, add_nacv=add_nacv, add_grad=add_grad, add_gap=add_gap
        )
