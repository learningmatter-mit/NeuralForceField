...
class ResidualMLP(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(
            residual, layer_types[activation](), Dense(in_features=feat_dim, out_features=feat_dim, bias=bias)
        )
...
class BatchedResidualMLP(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(residual, nonlinear, linear)
        self.num_out = num_out
...
class ElectronicEmbedding(nn.Module):
    ...
    def forward(self, psi, e_z, num_atoms):
        ...
        for j, mol_q in enumerate(split_qs):
            ...
            arg = torch.einsum("ij, jk -> i", mol_q, k) / self.feat_dim**0.5
...
class CombinedEmbedding(nn.Module):
    ...
    def forward(self, charge, spin, z, num_atoms):
        ...
        e_q = self.charge_embedding(psi=charge, e_z=e_z, num_atoms=num_atoms)
        e_s = self.charge_embedding(psi=spin, e_z=e_z, num_atoms=num_atoms)
...
class LocalInteraction(nn.Module):
    def __init__(
        ...
        self.resmlp = BatchedResidualMLP(
            feat_dim=feat_dim,
    ...
    def g_matmul(self, r_ij, orbital):
    ...
    def make_quant(
        ...
        per_nbr = res_out.reshape(n_nbrs, -1, 1) * matmul

        out = scatter_add(src=per_nbr, index=nbrs[:, 0], dim=0, dim_size=graph_size)

        return out

    def take_inner(self, quant, orbital):
        name = orbital.upper()
    ...
    def forward(self, xyz, x_tilde, nbrs):
        ...
        for quant, orbital in zip(quants[1:], orbitals[1:]):
            invariant = self.take_inner(quant, orbital)
...
class NonLocalInteraction(nn.Module):
    def __init__(
        ...
        # as wrappers
        self.attn = FastAttention(dim_heads=feat_dim, nb_features=feat_dim, causal=False)
        self.feat_dim = feat_dim
        self.resmlp = BatchedResidualMLP(
            feat_dim=feat_dim, num_out=3, activation=activation, dropout=dropout, residual_layers=residual_layers
        )
...
class InteractionBlock(nn.Module):
    def __init__(
        ...
        self.non_local = NonLocalInteraction(feat_dim=feat_dim, activation=activation, dropout=dropout)

    def forward(self, x, xyz, nbrs, num_atoms):
        x_tilde = self.residual_1(x)
        l = self.local(xyz=xyz, x_tilde=x_tilde, nbrs=nbrs)
...
class Electrostatics(nn.Module):
    ...
    def get_en(self, q, xyz, num_atoms, mol_nbrs):
    ...
    def forward(self, f, z, xyz, total_charge, num_atoms, mol_nbrs):
        q = self.get_charge(f=f, z=z, total_charge=total_charge, num_atoms=num_atoms)
        energy = self.get_en(q=q, xyz=xyz, num_atoms=num_atoms, mol_nbrs=mol_nbrs)
...
class NuclearRepulsion(nn.Module):
    ...
    def zbl_phi(self, r_ij, z_i, z_j):
        ...
        a = (d / (z_i**z_exp + z_j**z_exp)).reshape(-1)

        out = (c * torch.exp(-exponents * r_ij.reshape(-1) / a)).sum(0)

        return out

    def forward(self, xyz, z, nbrs, num_atoms):
