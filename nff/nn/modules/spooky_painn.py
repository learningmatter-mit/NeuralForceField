...
from nff.nn.modules.painn import InvariantMessage, norm, preprocess_r
...
from nff.utils.constants import BOHR_RADIUS, KE_KCAL
...
class NonLocalInteraction(nn.Module):
    def __init__(self, feat_dim, activation, nb_features=None, dropout=DEFAULT_DROPOUT):
        ...
        self.dense = Dense(in_features=feat_dim, out_features=(3 * feat_dim), bias=True, dropout_rate=dropout)
    ...
    def pad(self, Q, K, V, num_atoms):
        ...
        for i, q in enumerate(q_split):
            ...
            pad = -20 * torch.ones(extra_pad, self.feat_dim, device=Q.device)
    ...
    def forward(self, s, num_atoms):
        Q, K, V = torch.split(self.dense(s), [self.feat_dim] * 3, dim=-1)
...
class MessageBlock(nn.Module):
    def __init__(self, feat_dim, activation, n_rbf, cutoff, learnable_k, dropout, fast_feats):
        super().__init__()
        ...
        self.nl = NonLocalInteraction(feat_dim=feat_dim, activation=activation, dropout=dropout, nb_features=fast_feats)

    def forward(self, s_j, v_j, r_ij, nbrs, num_atoms, **kwargs):
        ...
        delta_s_l = scatter_add(src=delta_s_ij, index=nbrs[:, 0], dim=0, dim_size=graph_size)
        delta_s_nl = self.nl(s=s_j, num_atoms=num_atoms)
...
class GatedEquivariant(nn.Module):
    def __init__(self, feat_dim, activation, dropout=DEFAULT_DROPOUT):
        super().__init__()

        self.feat_dim = feat_dim
        self.W_1 = Dense(in_features=feat_dim, out_features=feat_dim, bias=False, dropout_rate=dropout)
        self.W_2 = Dense(in_features=feat_dim, out_features=1, bias=False, dropout_rate=dropout)
        self.block = nn.Sequential(
            Dense(in_features=(2 * feat_dim), out_features=feat_dim, bias=True, dropout_rate=dropout),
            layer_types[activation](),
            Dense(in_features=feat_dim, out_features=2, bias=True, dropout_rate=dropout),
        )
    ...
    def forward(self, s_i, v_i):
        ...
        w_v_1 = torch.einsum("ij, kjl -> kil", self.W_1.weight, v_i)
        w_v_2 = torch.einsum("ij, kjl -> kil", self.W_2.weight, v_i)
...
class GatedInvariant(nn.Module):
    def __init__(self, feat_dim, activation, dropout=DEFAULT_DROPOUT):
        ...
        self.block = nn.Sequential(
            Dense(in_features=(2 * feat_dim), out_features=feat_dim, bias=True, dropout_rate=dropout),
            layer_types[activation](),
            Dense(in_features=feat_dim, out_features=1, bias=True, dropout_rate=dropout),
        )
...
class Electrostatics(nn.Module):
    def __init__(
        ...
        gate_class = GatedEquivariant if point_dipoles else GatedInvariant
        self.gated = gate_class(feat_dim=feat_dim, activation=activation, dropout=dropout)
        self.r_on = r_cut / 4
    ...
    def charge_and_dip(self, xyz, s_i, v_i, z, total_charge, num_atoms):
        atom_charges, atom_dipoles = self.gated(s_i=s_i, v_i=v_i)
        mol_sum = scatter_mol(atomwise=atom_charges, num_atoms=num_atoms).reshape(-1)
        ...
        summed_chg_dipole = get_dipole(xyz=xyz, q=atom_charges, num_atoms=num_atoms)

        full_dipole = summed_chg_dipole
        if self.point_dipoles:
            summed_atomwise_dipole = scatter_mol(atomwise=atom_dipoles, num_atoms=num_atoms)

            full_dipole = full_dipole + summed_atomwise_dipole

        return atom_charges, atom_dipoles, full_dipole

    def get_charge_charge(self, dist, q, num_atoms, mol_nbrs):
        q_i = q[mol_nbrs[:, 0]].reshape(-1)
        q_j = q[mol_nbrs[:, 1]].reshape(-1)

        arg_0 = self.f_switch(dist) / (dist**2 + BOHR_RADIUS**2) ** 0.5
    ...
    def get_charge_dip(self, unit_r_ij, dist, q, dip, num_atoms, mol_nbrs):
        ...
        arg_0 = self.f_switch(dist) / (dist**2 + BOHR_RADIUS**2)
    ...
    def get_dip_dip(self, unit_r_ij, dist, q, dip, num_atoms, mol_nbrs):
        ...
        numerator = (dip_i * dip_j).sum(-1) - 3 * (unit_r_ij * dip_i).sum(-1) * (unit_r_ij * dip_j).sum(-1)

        arg_0 = self.f_switch(dist) / (dist**2 + BOHR_RADIUS**2) ** (3 / 2)
    ...
    def get_en(self, unit_r_ij, dist, q, dip, num_atoms, mol_nbrs):
        en = self.get_charge_charge(dist=dist, q=q, num_atoms=num_atoms, mol_nbrs=mol_nbrs)

        if not self.point_dipoles:
            return en

        if self.charge_dipole:
            c_d = self.get_charge_dip(
                unit_r_ij=unit_r_ij, dist=dist, q=q, dip=dip, num_atoms=num_atoms, mol_nbrs=mol_nbrs
            )

            en = en + c_d

        if self.dipole_dipole:
            d_d = self.get_dip_dip(unit_r_ij=unit_r_ij, dist=dist, q=q, dip=dip, num_atoms=num_atoms, mol_nbrs=mol_nbrs)

            en = en + d_d

        return en

    def forward(self, s_i, v_i, z, xyz, total_charge, num_atoms, mol_nbrs, mol_offsets):
        idx = mol_nbrs[:, 1] > mol_nbrs[:, 0]
        mol_nbrs = mol_nbrs[idx]
        mol_offsets = mol_offsets[idx]

        q, dip_atom, full_dip = self.charge_and_dip(
            xyz=xyz, s_i=s_i, v_i=v_i, z=z, total_charge=total_charge, num_atoms=num_atoms
        )
        ...
        energy = self.get_en(unit_r_ij=unit_r_ij, dist=dist, q=q, dip=dip_atom, num_atoms=num_atoms, mol_nbrs=mol_nbrs)
...
class ElectronicEmbedding(nn.Module):
    def __init__(self, feat_dim, activation, dropout=DEFAULT_DROPOUT):
        super().__init__()
        self.linear = Dense(
            in_features=feat_dim, out_features=feat_dim, bias=True, activation=None, dropout_rate=dropout
        )
        self.feat_dim = feat_dim
        self.dense = Dense(
...
class CombinedEmbedding(nn.Module):
    def __init__(self, feat_dim, activation, max_z=DEFAULT_MAX_Z, dropout=DEFAULT_DROPOUT):
        super().__init__()
        self.feat_dim = feat_dim
        self.nuc_embedding = NuclearEmbedding(feat_dim=feat_dim, max_z=max_z)
        self.charge_embedding = ElectronicEmbedding(feat_dim=feat_dim, activation=activation, dropout=dropout)
        self.spin_embedding = ElectronicEmbedding(feat_dim=feat_dim, activation=activation, dropout=dropout)
    ...
    def forward(self, charge, spin, z, num_atoms):
        ...
        s_i = e_z + e_q + e_s
        v_i = torch.zeros(num_atoms.sum(), self.feat_dim, 3).to(s_i.device)
