...
def init_calculator(atoms, params):
    ...
    model = load_model(weightpath, model_type=model_type, params=nn_params)
...
def get_output_keys(model):
    ...
    readout_dict_names = [name for name in readout_attr_names if type(getattr(atomwisereadout, name)) is ModuleDict]
...
def get_loader(model, nxyz_list, num_states, cutoff, needs_angles=False, base_keys=["energy"]):
    ...
    ref_quant_grad = [np.zeros(((len(nxyz_list[0])), 3)).tolist()] * len(nxyz_list)
...
def check_convg(model, loader, energy_key, device, restart_file):
    mode_dic = get_modes(model=model, loader=loader, energy_key=energy_key, device=device)
...
def get_opt_kwargs(params):
    ...
    opt_kwargs = {key: val for key, val in new_params.items() if key in OPT_KEYS}
...
def opt_conformer(atoms, params):
    ...
    for _iteration in tqdm(range(max_rounds)):
        ...
        hess_converged, mode_dic = check_convg(
            model=model, loader=loader, energy_key=energy_key, device=device, restart_file=restart_file
        )
...
def md_to_conf(params):
    thermo_filename = params.get("thermo_filename", nve.DEFAULTNVEPARAMS["thermo_filename"])
    ...
    nve_instance = nve.Dynamics(atomsbatch=atoms, mdparam=nve_params)
    nve_instance.run()

    thermo_filename = params.get("thermo_filename", nve.DEFAULTNVEPARAMS["thermo_filename"])
    traj_filename = params.get("traj_filename", nve.DEFAULTNVEPARAMS["traj_filename"])

    num_starting_poses = params.get("num_starting_poses", NUM_CONFS)
    best_confs = get_confs(
        traj_filename=traj_filename, thermo_filename=thermo_filename, num_starting_poses=num_starting_poses
    )
...
def get_opt_and_modes(params):
    ...
    all_geoms, all_modes = confs_to_opt(params=params, best_confs=best_confs)
...
def get_orth(mass_vec, matrix):
    ...
    sqrt_m_mat = np.kron(sqrt_m_vec, np.ones((1, len(sqrt_m_vec))))
...
def make_dx_dp(mass_vec, cc_matrix, cc_freqs, kt=25.7 / 1000 / 27.2, hb=1, classical=False):
    ...
    dx, dp = get_disp(mass_vec=mass_vec, matrix=matrix, freqs=freqs, q=unitless_x, p=unitless_p, hb=hb)
...
def split_convert_xyz(xyz):
    ...
    mass_vec = [PERIODICTABLE.GetAtomicWeight(int(element[0])) * AMU_2_AU for element in xyz]
...
def make_wigner_init(init_atoms, vibdisps, vibfreqs, num_samples, kt=25.7 / 1000 / 27.2, hb=1, classical=False):
    ...
    xyz = np.concatenate([init_atoms.get_atomic_numbers().reshape(-1, 1), init_atoms.get_positions()], axis=1)
    atoms_list = []

    for _ in range(num_samples):
        assert min(vibfreqs) >= 0, "Negative frequencies found. " "Geometry must not be converged."

        Z, opt_coords, mass_vec = split_convert_xyz(xyz)
        dx, dp = make_dx_dp(mass_vec, vibdisps, vibfreqs, kt, hb, classical=classical)
...
def nms_sample(params, classical, num_samples, kt=25.7 / 1000 / 27.2, hb=1):
...
def get_modes(model, loader, energy_key, device):
    ...
    if isinstance(model, SchNet):
        hessian = (
            get_schnet_hessians(batch=batch, model=model, device=device, energy_key=energy_key)[0]
            .cpu()
            .detach()
            .numpy()
        )
    ...
    force_consts, vib_freqs, eigvec = vib_analy(
        r=batch["nxyz"][:, 0].cpu().detach().numpy(), xyz=batch["nxyz"][:, 1:].cpu().detach().numpy(), hessian=hessian
    )

    # from https://gaussian.com/vib/#SECTION00036000000000000000
    nxyz = batch["nxyz"].cpu().detach().numpy()
    masses = np.array([PT.GetMostCommonIsotopeMass(int(z)) for z in nxyz[:, 0]])
...
def moi_tensor(massvec, expmassvec, xyz):
    ...
    mass_xyz_com_sq_sum = np.sum(expmassvec.reshape(-1, 3) * xyz_com**2, axis=0)
    ...
    moi = np.array(
        [
            [mass_xyz_com_sq_sum[1] + mass_xyz_com_sq_sum[2], -1 * mass_xy, -1 * mass_xz],
            [-1 * mass_xy, mass_xyz_com_sq_sum[0] + mass_xyz_com_sq_sum[2], -1 * mass_yz],
...
def trans_rot_vec(massvec, xyz_com, moi_eigvec):
    ...
    dx_norms = np.stack((d1_norm, d2_norm, d3_norm, d4_norm, d5_norm, d6_norm))
...
def vib_analy(r, xyz, hessian):
    ...
    hess_proj = np.einsum("i,ij,j->ij", 1 / sqrtinvmassvec, mwhess_proj, 1 / sqrtinvmassvec)
    ...
    return (force_constants_J_m_2, proj_vib_freq_cm_1, proj_hessian_eigvec, mwhess_proj, hess_proj)
...
def low_freq_entropy(freqs, temperature, b_av=B_AV):
...
def high_freq_entropy(freqs, temperature):
    freq_ev = freqs * CM_TO_EV
    exp_pos = np.exp(freq_ev / (kB * temperature)) - 1
    exp_neg = 1 - np.exp(-freq_ev / (kB * temperature))

    entropy = GAS_CONST * (freq_ev / (kB * temperature * exp_pos) - np.log(exp_neg))

    return entropy


def mrrho_entropy(freqs, temperature, rotor_cutoff, b_av, alpha):
    func = 1 / (1 + (rotor_cutoff / freqs) ** alpha)
    s_r = low_freq_entropy(freqs=freqs, b_av=b_av, temperature=temperature)
    s_v = high_freq_entropy(freqs=freqs, temperature=temperature)
...
def mrrho_quants(
    ...
    old_entropy = ideal_gas.get_entropy(temperature=temperature, pressure=pressure).item()
    enthalpy = ideal_gas.get_enthalpy(temperature=temperature).item()

    # correction to vibrational entropy
    out = mrrho_entropy(freqs=abs_freqs, temperature=temperature, rotor_cutoff=rotor_cutoff, b_av=b_av, alpha=alpha)
...
def convert_modes(atoms, modes):
    masses = atoms.get_masses().reshape(-1, 1).repeat(3, 1).reshape(1, -1)
...
def hessian_and_modes(
    ...
    if analytical:
        ...
        dim = len(ase_atoms)
        hessian = vib_results.get_hessian().reshape(dim * 3, dim * 3) * EV_TO_AU * BOHR_RADIUS**2

        print(vib.get_frequencies()[:20])

    vib_results = vib_analy(r=ase_atoms.get_atomic_numbers(), xyz=ase_atoms.get_positions(), hessian=hessian)
    _, freqs, modes, mwhess_proj, hess_proj = vib_results
    mwhess_proj *= AMU2KG

    vibdisps = convert_modes(atoms=ase_atoms, modes=modes)

    mrrho_results = mrrho_quants(
        ase_atoms=ase_atoms,
        freqs=freqs,
        imag_cutoff=imag_cutoff,
