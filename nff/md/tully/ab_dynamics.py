...
def load_params(file):
    ...
    all_params["nacv_details"] = {**all_params, **all_params["nacv_details"]}
    all_params["grad_details"] = {**all_params, **all_params["grad_details"]}
...
class AbTully(NeuralTully):
    ...
    @property
    def forces(self):
        inf = np.ones((self.num_atoms, 3)) * float("inf")
        _forces = np.stack([-self.props.get(f"energy_{i}_grad", inf).reshape(-1, 3) for i in range(self.num_states)])
    ...
    def correct_phase(self, old_force_nacv):
        ...
        delta = np.max(np.linalg.norm(old_force_nacv - new_force_nacv, axis=((-1, -2))), axis=-1)
        sigma = np.max(np.linalg.norm(old_force_nacv + new_force_nacv, axis=((-1, -2))), axis=-1)
    ...
    def update_props(self, *args, **kwargs):
    ...
    def new_force_calc(self):
        ...
        surf = self.surfs[0]
        needs_calc = np.bitwise_not(np.isfinite(self.forces[0, surf])).any()

        if not needs_calc:
            return

        new_job_dir = os.path.join(os.getcwd(), f"{self.step_num - 1}_extra")
    ...
    def from_file(cls, file):
        ...
        instance = cls(atoms_list=atoms_list, **all_params)
