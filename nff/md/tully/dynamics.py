...
from nff.md.tully.io import get_atoms, get_results, load_json
...
class NeuralTully:
    def __init__(
        ...
        **kwargs,
        ...
        self.surfs = np.ones(self.num_samples, dtype=np.int) * initial_surf
    ...
    def init_sigma(self):
        ...
        sigma = np.ones((self.num_samples, self.num_states, self.num_states), dtype=np.complex128)

        sigma = self.c.reshape(self.num_samples, self.num_states, 1)
        sigma *= np.conj(self.c.reshape(self.num_samples, 1, self.num_states))
    ...
    @property
    def forces(self):
        _forces = np.stack([-self.props[f"energy_{i}_grad"] for i in range(self.num_states)], axis=1)
    ...
    @property
    def energy(self):
        _energy = np.stack([self.props[f"energy_{i}"].reshape(-1) for i in range(self.num_states)], axis=1)
    ...
    def full_energy(self):
        _energy = np.stack([self.props[f"energy_{i}"].reshape(-1) for i in range(self.num_diabat)], axis=1)
    ...
    def full_pot_V(self):
        ...
        V = np.zeros((self.num_samples, self.num_diabat, self.num_diabat))
        idx = np.arange(self.num_diabat)
        np.put_along_axis(
            V, idx.reshape(1, -1, 1), self.full_energy.reshape(self.num_samples, self.num_diabat, 1), axis=2
        )
    ...
    def H_d(self):
        ...
        for i in range(self.num_diabat):
            for j in range(self.num_diabat):
                ...
                _H_d[..., i, j] = self.props[diabat_key].reshape(-1)
    ...
    def log(self):
        ...
        text = self.log_template % (time, *pcts, norm_c, p_avg)
    ...
    def update_props(self, needs_nbrs):
        ...
        # this is being used
        needs_nacv = ("subotnik" in self.decoherence_type) or (not self.diabat_propagate) or (self.hop_eqn == "tully")
    ...
    def do_hop(self, old_c, P):
        self.p_hop = get_p_hop(
            hop_eqn=self.hop_eqn,
            old_c=old_c,
            new_c=self.c,
            P=P,
            surfs=self.surfs,
    ...
    def add_decoherence(self):
        ...
        self.c = self.decoherence(c=self.c, surfs=self.surfs, energy=energy, vel=self.vel, dt=self.dt, mass=self.mass)
    ...
    def step(self, needs_nbrs):
        ...
        new_xyz, new_vel = verlet_step_1(
            self.forces, self.surfs, vel=self.vel, xyz=self.xyz / const.BOHR_RADIUS, mass=self.mass, dt=self.dt
        )
        self.xyz = new_xyz * const.BOHR_RADIUS
        self.vel = new_vel

        self.update_props(needs_nbrs)
        new_vel = verlet_step_2(forces=self.forces, surfs=self.surfs, vel=self.vel, mass=self.mass, dt=self.dt)
        self.vel = new_vel
        ...
        if self.nacv is not None:
            self.T, _ = compute_T(nacv=self.nacv, vel=self.vel, c=self.c)
        new_surfs, new_vel = self.do_hop(old_c=old_c, P=P)
...
def main():
    ...
    parser.add_argument("--params_file", type=str, help="Info file with parameters", default="job_info.json")
