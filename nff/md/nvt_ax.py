...
class NoseHooverChain(MolecularDynamics):
    def __init__(
        ...
        **kwargs,
        ...
        MolecularDynamics.__init__(self, atoms, timestep * units.fs, trajectory, logfile, loginterval)
    ...
    def get_zeta_accel(self):
        p0_dot = 2 * (self.atoms.get_kinetic_energy() - self.targeEkin) - self.p_zeta[0] * self.p_zeta[1] / self.Q[1]
        p_middle_dot = self.p_zeta[:-2] ** 2 / self.Q[:-2] - self.T - self.p_zeta[1:-1] * self.p_zeta[2:] / self.Q[2:]
    ...
    def step(self):
        ...
        accel = self.atoms.get_forces() / self.atoms.get_masses().reshape(-1, 1)
        v_full_system = (v_half_system + 1 / 2 * accel * self.dt) / (1 + 0.5 * self.dt * v_full_zeta[0])
