...
def get_adjacency_matrix(xyz, atom_numbers, bond_length, oxygeninvolved, cell=None, device='cpu'):
    ...
    adjacency_matrix = adjacency_matrix[torch.where(
        atom_numbers == 14)[0].view(-1, 1), torch.where(atom_numbers == 8)[0][oxygeninvolved]]
    ...
    adjacency_matrix = adjacency_matrix[torch.where(adjacency_matrix >= 0)[
        0], torch.where(adjacency_matrix >= 0)[1]].view(-1, 1)
    ...
    return torch.LongTensor(edges).to(device), torch.LongTensor(atomslist).to(
        device), [(len(atomslist))], adjacency_matrix.float().to(device)
...
def get_molecules(atom, bond_length, mode='bond', periodic=True):
    ...
    for i in range(xyz.shape[0]):
        ...
        if mode == 'bond':
            for t in types:
                if bond_length.get('%s-%s' % (ty, t)) is not None:
                    oxy_neighbors.extend(list(np.where(atom.numbers == t)[0][np.where(
                        dis_sq[i, np.where(atom.numbers == t)[0]] <= bond_length['%s-%s' % (ty, t)])[0]]))
        elif mode == 'cutoff':
        ...
        if (clusters[oxy_neighbors] == 0).all() and clusters[i] != 0:
            ...
            tmp = clusters[oxy_neighbors][clusters[oxy_neighbors] != 0][clusters[oxy_neighbors][
                clusters[oxy_neighbors] != 0] != min(clusters[oxy_neighbors][clusters[oxy_neighbors] != 0])]
            clusters[i] = min(clusters[oxy_neighbors][clusters[oxy_neighbors] != 0])
            clusters[oxy_neighbors] = min(clusters[oxy_neighbors][clusters[oxy_neighbors] != 0])
            for tr in tmp:
