...
class NoseHooverNPT(MolecularDynamics):
    ...
    def step(self):
        ...
        vel_half = np.exp(self.eta - eta0) * (
            vel + 0.5 * self.dt * (accel - self.zeta * vel - (2 + (3 / self.Nf)) * vel * self.veta)
        )
...
class NoseHooverChainsNPT_Hydrostatic(MolecularDynamics):
    ...
    def step(self):
        ...
        # (eq D5)
        F_eps = (
            self.d * V * (P_hyd - self.P_ext)  # only considering hydrostatic
            + (self.d / self.N_dof) * 2.0 * Ekin
        )

        dpzeta_dt = np.zeros(shape=self.p_zeta.shape)
        dpzeta_dt[0] = (
            2 * (Ekin - self.targetEkin)
            + self.W * (self.veps**2)
            - self.T
        ...
        delta_eps = self.veps * self.dt + 0.5 * (F_eps / self.W - self.veps * self.p_zeta[0] / self.Q[0]) * self.dt**2
        ...
        coupl_accel = accel - vel * self.p_zeta[0] / self.Q[0] - (2.0 + self.d / self.N_dof) * vel * self.veps
        vel_half = scale_coords * (vel + 0.5 * self.dt * coupl_accel)
        self.veps += 0.5 * self.dt * (F_eps / self.W - self.veps * self.p_zeta[0] / self.Q[0])
        ...
        coupl_accel = accel - vel_half * self.p_zeta[0] / self.Q[0] - (2.0 + self.d / self.N_dof) * vel * self.veps
        vel_guess = vel_half + 0.5 * self.dt * coupl_accel
        veps_guess = self.veps + 0.5 * self.dt * (F_eps / self.W - self.veps * self.p_zeta[0] / self.Q[0])
        ...
        while max_rel_del > 1e-5 and count < 100:
            ...
            vel_guess = vel_half + (
                accel - vel_half * p_zeta_guess[0] / self.Q[0] - (2.0 + self.d / self.N_dof) * vel_half * veps_guess
            ) * 0.5 * self.dt / (
                1.0 + 0.5 * self.dt * (p_zeta_guess[0] / self.Q[0] + (2.0 + self.d / self.N_dof) * veps_guess)
            )
            ...
            veps_guess = self.veps + 0.5 * self.dt * (F_eps / self.W - self.veps * p_zeta_guess[0] / self.Q[0]) / (
    ...
    def step(self):
        ...
        h0_t = np.matmul(
            (
                ...
                * (
                    ...
                    + np.power(
                        self.vg, 2
                    )  # np.matmul(self.vg, self.vg) # not sure whether this is the square or matrix square!
        ...
        while np.isclose(1.0, np.linalg.det(h0_t), atol=1e-6) is False:
            ...
            if np.linalg.det(h0_t) is np.nan:
                ...
                exit()
            h0_t += (
                h0 * (np.linalg.det(h0_t) - 1.0) / (np.linalg.det(h0_t) * np.trace(np.matmul(np.linalg.inv(h0_t), h0)))
            )
        ...
        coupl_accel = (
            ...
            - (2.0 + self.d / self.N_dof) * vel * self.veps
        )
        vel_half = scale_coords * np.matmul(np.matmul(h0_t, np.linalg.inv(h0)), (vel + 0.5 * self.dt * coupl_accel).T).T
        self.veps += 0.5 * self.dt * (F_eps / self.W - self.veps * self.p_zeta[0] / self.Q[0])
        self.vg += (
            0.5
            * self.dt
        ...
        x_t = (
            scale_coords
            * np.matmul(np.matmul(h0_t, np.linalg.inv(h0)), (x + (vel + 0.5 * self.dt * coupl_accel) * self.dt).T).T
        )
        ...
        while max_rel_del > 1e-5 and count < 100:
            count += 1
            if (
                np.isnan(veps_guess).any()
                or np.isnan(vg_guess).any()
                or np.isnan(p_zeta_guess).any()
                or np.isnan(vel_guess).any()
            ):
                print("Some velocities diverged!")
                exit()
            old_guess = copy.deepcopy(vel_guess)
            ...
            vel_guess = vel_half + (
                ...
                - (2.0 + self.d / self.N_dof) * vel_half * veps_guess
                - 2.0 * np.matmul(vg_guess, vel_half.T).T
            ) * 0.5 * self.dt / (
                1.0
                + 0.5
                * self.dt
                * (
                    ...
                    + (2.0 + self.d / self.N_dof) * veps_guess
                    + 2.0 * np.matmul(vg_guess, vel_half.T).T / vel_half
                )
            )
            self.atoms.set_velocities(vel_guess)
            Ekin = self.atoms.get_kinetic_energy()
            ...
            vg_guess = self.vg + 0.5 * self.dt * (
                F_g / self.Wg + self.vg * vg_guess - self.vg * p_zeta_guess[0] / self.Q[0]
            ) / (1.0 + 0.5 * self.dt * (p_zeta_guess[0] / self.Q[0] - vg_guess))
