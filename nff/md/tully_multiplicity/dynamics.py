...
from nff.md.tully_multiplicity.step import (try_hop,
...
class NeuralTully:
    ...
    def get_SOC_mat(self):
        ...
        H_soc = np.zeros((self.num_samples,
                          self.num_states,
                          self.num_states),
                         dtype=np.complex128)
    ...
    def get_neg_G_hmc(self):
        ...
        np.put_along_axis(
            ...
            self.forces.reshape(self.num_samples,
                                self.num_states,
                                1,
                                self.num_atoms,
                                3),
    ...
    def get_diag_energy(self):
        ...
        _energy = np.take_along_axis(np.real(H_diag),
                                     idxs.reshape(1, -1, 1), axis=2)
...
#             pcts.append(pct)
    ...
    def update_props(self,
        ...
        props = get_results(models=self.models,
                            nxyz=self.nxyz,
...
if not isinstance(self.U_old, type(None)):
...
#                 print(preserved_idxs)
    ...
    def do_hop(self,
        ...
        new_surfs, new_vel = try_hop(p_hop=self.p_hop,
                                     surfs=self.surfs,
                                     vel=self.vel,
                                     nacv=self.nacv,
                                     mass=self.mass,
    ...
    def add_decoherence(self):
        ...
        self.c_diag = self.decoherence(c=self.c_diag,
                                       surfs=self.surfs,
                                       energy=self.diag_energy,
                                       vel=self.vel,
                                       dt=self.dt,
                                       mass=self.mass)
    ...
    def step(self, needs_nbrs):
        ...
        new_xyz, new_vel = verlet_step_1(self.diag_forces,
                                         self.surfs,
                                         vel=self.vel,
        ...
        new_vel = verlet_step_2(forces=self.diag_forces,
                                surfs=self.surfs,
                                vel=self.vel,
                                mass=self.mass,
                                dt=self.dt)
