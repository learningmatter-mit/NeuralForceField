...
- `model_names` (list[str]): Names of the different ChemProp models. They are saved in `model_folder_cp/model_name` for each of the model names.
- `device` (Union[str, int]): device to evaluate the model on
- `smiles_folder` (str): Folder with the smiles files.
