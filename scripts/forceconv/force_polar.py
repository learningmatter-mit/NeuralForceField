...
class ForcePai_polar(nn.Module):
    def __init__(self,
        ...
        # self.edge_readout = nn.ModuleList(
        #     [nn.Sequential(Dense(in_features=feat_dim, 
        #                          out_features=feat_dim,
        #                          bias=True,
        #                          dropout_rate=conv_dropout,
    ...
    def atomwise(self,
        ...
        for i, message_block in enumerate(self.message_blocks):
            ...
            # de_update = edge_update_block( (s_i[nbrs[:, 0]] * v_i[nbrs[:, 1]]).sum(-1) )
            
            # fqq = (s_i[nbrs[:, 0]] * s_i[nbrs[:, 1]]).mean(-1)[..., None] * xyz_adjoint # / dis.unsqueeze(-1) ** 2
            
            fqq = (s_i[nbrs[:, 0]] * s_i[nbrs[:, 1]]).mean(-1).unsqueeze(-1) * xyz_adjoint / dis.unsqueeze(-1) ** 2
            
            fqp1 = -(s_i[nbrs[:, 0]].unsqueeze(-1) * v_i[nbrs[:, 1]]).mean(1)  / dis.unsqueeze(-1) ** 3
            
            pjr_dot = v_i[nbrs[:,1]] * xyz_adjoint.unsqueeze(1)
            pir_dot = v_i[nbrs[:,0]] * xyz_adjoint.unsqueeze(1)
            
            pipj = (v_i[nbrs[:,1]] *  v_i[nbrs[:,0]]).sum(-1)
            
            fqp2 = 3 * (s_i[nbrs[:, 0]] * pjr_dot.sum(-1)).mean(1).unsqueeze(-1) * xyz_adjoint / dis.unsqueeze(-1) ** 3
             
            
            # pipj = (v_i[nbrs[:,1]] *  v_i[nbrs[:,0]]).sum(-1)
    
            # fpp1 = (3 / dis.unsqueeze(-1) **4) * pipj.mean(-1)[..., None] * xyz_adjoint
        
            # fpp2 = -(15 / dis.unsqueeze(-1)**4 )  * (pjr_dot * pjr_dot).sum(-1).mean(-1)[..., None] * xyz_adjoint
    
            # fpp3 = (3 / dis.unsqueeze(-1)**4 ) * ( pir_dot.sum(-1)[..., None] * v_i[nbrs[:,1]] + pjr_dot.sum(-1)[..., None] * v_i[nbrs[:,0]]).mean(1)
