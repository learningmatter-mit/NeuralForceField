...
class ForcePai_res(nn.Module):
    def __init__(self,
        ...
        # edge readout 
        self.edgereadout = nn.ModuleList( [
            Sequential(
            Dense(in_features=feat_dim, 
                  out_features=feat_dim, 
                  bias=True, 
            ...
            Dense(in_features=feat_dim, 
                  out_features=1, 
                  bias=True,
                  dropout_rate=readout_dropout)) for _ in range(num_conv)]
        )
        # self.edgereadout = EdgeReadoutBlock(feat_dim=feat_dim,
        #                                     activation=activation,
        #                                     dropout=readout_dropout)
    ...
    def atomwise(self,
        ...
        for i, message_block in enumerate(self.message_blocks):
            ...
            f_ij = self.edgereadout[i](e_ij) * xyz_adjoint

            if i == 0:
              f = scatter_add(f_ij, nbrs[:,0], dim=0, dim_size=graph_size)
            else:
              f += scatter_add(f_ij, nbrs[:,0], dim=0, dim_size=graph_size)

        results['energy_grad'] = f
...
class ForcePai_polar(nn.Module):
    ...
    def atomwise(self,
        ...
        for i, message_block in enumerate(self.message_blocks):
            ...
            # (1 x M x 3) * (M x 3 x Ne), Ne: # of edges 
            # (1 x M) * (M x Ne)

            # xyz_adjoint = d|v_ij|/dxyz =  v_ij /||v_ij||

            f_ij_scale = ((ds_message_ij.unsqueeze(2) * xyz_adjoint.unsqueeze(1)) * dv_message_ij).sum(-1).sum(-1)
                            # Ne x M x 1                       # Ne x 1 x 3          => Ne x M x 3     => Ne     
            f_ij_scale += self.edgereadout[i](e_ij).squeeze()

            f_ij = f_ij_scale.unsqueeze(-1) * xyz_adjoint
